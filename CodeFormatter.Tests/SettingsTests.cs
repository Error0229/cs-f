...
public class SettingsTests
{
    ...
    // PHP (dprint mago)
    [InlineData(Language.Php, "tabWidth", 2, "<?php\nclass X{function m(){return 1;}}", "\n    return 1;")]
    [InlineData(Language.Php, "useTabs", true, "<?php\nclass X{function m(){return 1;}}", "\n\t\treturn 1;")]
    [InlineData(Language.Php, "printWidth", 20, "<?php\n$x = [1111111, 2222222, 3333333];", "[\n")]
    [InlineData(Language.Php, "endOfLine", "crlf", "<?php\n$a = 1;\n$b = 2;\n", "\r\n")]
    [InlineData(Language.Php, "singleQuote", false, "<?php\n$a = \"x\";", "\"x\"")]
    [InlineData(Language.Php, "methodBraceStyle", "same-line", "<?php\nclass X{function m(){return 1;}}", "m() {")]
    [InlineData(Language.Php, "followingClauseOnNewline", true, "<?php\nif ($a) {\nf();\n} else {\ng();\n}", "}\nelse {")]
    [InlineData(Language.Php, "spaceAroundConcatenationBinaryOperator", false, "<?php\n$a = $b . $c;", "$b.$c")]
    [InlineData(Language.Php, "sortUses", "preserve", "<?php\nuse B\\C;\nuse A\\B;\n", "use B\\C;\nuse A\\B;")]
    [InlineData(Language.Php, "_extra", "\"parenthesesInNewExpression\": false", "<?php\n$a = new Foo();", "new Foo;")]
    // Templates (dprint markup_fmt)
    [InlineData(Language.Jinja, "indentWidth", 4, "<ul>\n{% for i in items %}\n<li>{{ i }}</li>\n{% endfor %}\n</ul>", "\n        <li>")]
    [InlineData(Language.Twig, "quotes", "single", "<div class=\"a\">{{ x }}</div>", "class='a'")]
    [InlineData(Language.Handlebars, "indentWidth", 4, "<ul>\n{{#each items}}\n<li>{{name}}</li>\n{{/each}}\n</ul>", "\n        <li>")]
    [InlineData(Language.Angular, "html.void.selfClosing", "true", "<div>@if (a) {<br>}</div>", "<br />")]
    // LaTeX, BibTeX, Typst, Julia, CMake (dprint community plugins)
    [InlineData(Language.Latex, "indentWidth", 4, "\\begin{itemize}\n\\item a\n\\end{itemize}\n", "\n    \\item")]
    [InlineData(Language.Latex, "lineWidth", 20, "Some words that go on for a while here.\n", "go\non")]
    [InlineData(Language.Bibtex, "indentWidth", 4, "@book{a, title={B}}", "\n    title")]
    [InlineData(Language.Bibtex, "lowercase", false, "@BOOK{a, TITLE={B}}", "TITLE")]
    [InlineData(Language.Bibtex, "curly", true, "@book{a, title=\"B\"}", "{B}")]
    [InlineData(Language.Bibtex, "trailingCommas", true, "@book{a, title={B}}", "{B},\n")]
    [InlineData(Language.Typst, "indentWidth", 4, "#let f(x) = {\nx\n}", "\n    x")]
    [InlineData(Language.Typst, "lineWidth", 20, "#let x = (1111111, 2222222, 3333333)", "(\n")]
    [InlineData(Language.Typst, "lineEnding", "crlf", "= A\n\nb\n", "\r\n")]
    [InlineData(Language.Julia, "indentWidth", 2, "function f(x)\nreturn x\nend", "\n  return")]
    [InlineData(Language.Julia, "lineWidth", 20, "x = [1111111, 2222222, 3333333]", "[\n")]
    [InlineData(Language.CMake, "indentWidth", 4, "if(X)\nset(Y 1)\nendif()", "\n    set")]
    [InlineData(Language.CMake, "commandCase", "upper", "set(Y 1)", "SET(")]
    [InlineData(Language.CMake, "newLineKind", "crlf", "set(Y 1)\nset(Z 2)\n", "\r\n")]
